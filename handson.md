...
必須入力を`有効化`にします。Alexaの音声プロンプト部分に「メモする内容を言ってください。」と入力します。
...
「stat」と入力して［＋］をクリックします。プルダウンメニューは先程作成した`MemoSlot`を選択します。
...
`コードエディタ`タブをクリックします。`lambdda`フォルダを選択して、左上にある新規作成アイコンをクリックします。
